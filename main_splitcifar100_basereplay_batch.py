...
class Seq_Kprior(nn.Module):
    ...
    def loss_theta(self, phi_new, label_new, memory_bank_list = [], task_idx= None):
        ...
        if len(memory_bank_list) == 0:
            ...
            replay = 0.5*self.delta* l2_reg
        else:
            replay = 0
            for i_bank in memory_bank_list:
                Z_old,Y_old,task_idx_old = i_bank            
                f_old  = self.model(Z_old.to(phi_new.device))
                replay += costfunc_data(f_old,Y_old.to(phi_new.device)).sum()
           
        return loss_new , replay
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method',      type=str, default='replay', help='algo name - default') 
